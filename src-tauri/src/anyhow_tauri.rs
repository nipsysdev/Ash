// This code is based on the anyhow-tauri crate by Tomáš Diblík <dibla.tomas@post.cz>
// Repository: https://github.com/TDiblik/anyhow-tauri
// License: MIT
//
// The original code was inspired by @kesomannen's gale project:
// https://github.com/Kesomannen/gale/blob/7d05fa16c3497dce002e3b77b29e2ec922fa1ad7/src-tauri/src/util/cmd.rs
// Which is licensed under GPL-3.0, but he has permitted this code to be used under MIT.
...
// Add additional From implementations for common error types
impl From<hyper::http::uri::InvalidUri> for TACommandError {
    fn from(error: hyper::http::uri::InvalidUri) -> Self {
        Self(anyhow::anyhow!(error))
    }
}

impl From<hyper::header::InvalidHeaderValue> for TACommandError {
    fn from(error: hyper::header::InvalidHeaderValue) -> Self {
        Self(anyhow::anyhow!(error))
    }
}

impl From<hyper::http::uri::InvalidUriParts> for TACommandError {
    fn from(error: hyper::http::uri::InvalidUriParts) -> Self {
        Self(anyhow::anyhow!(error))
    }
}

impl From<hyper::Error> for TACommandError {
    fn from(error: hyper::Error) -> Self {
        Self(anyhow::anyhow!(error))
    }
}

impl From<hyper::http::Error> for TACommandError {
    fn from(error: hyper::http::Error) -> Self {
        Self(anyhow::anyhow!(error))
    }
}

impl From<hyper::http::header::ToStrError> for TACommandError {
    fn from(error: hyper::http::header::ToStrError) -> Self {
        Self(anyhow::anyhow!(error))
    }
}

impl From<hyper::http::status::InvalidStatusCode> for TACommandError {
    fn from(error: hyper::http::status::InvalidStatusCode) -> Self {
        Self(anyhow::anyhow!(error))
    }
}

impl From<serde_json::Error> for TACommandError {
    fn from(error: serde_json::Error) -> Self {
        Self(anyhow::anyhow!(error))
    }
}

impl From<tauri::Error> for TACommandError {
    fn from(error: tauri::Error) -> Self {
        Self(anyhow::anyhow!(error))
    }
}

impl From<pmtiles::PmtError> for TACommandError {
    fn from(error: pmtiles::PmtError) -> Self {
        Self(anyhow::anyhow!(error))
    }
}

impl From<std::io::Error> for TACommandError {
...
/// ```
/// #[tauri::command]
/// fn test() -> crate::anyhow_tauri::TAResult<String> {
    ...
    E: Into<anyhow::Error>,
{
    ...
    /// ```
    /// #[tauri::command]
    /// fn test_into_ta_result() -> crate::anyhow_tauri::TAResult<String> {
...
pub trait IntoEmptyTAResult<T> {
    ...
    /// ```
    /// #[tauri::command]
    /// fn test_into_ta_empty_result() -> crate::anyhow_tauri::TAResult<()> {
...
/// ```
/// #[tauri::command]
/// fn test_bail() -> crate::anyhow_tauri::TAResult<String> {
///     crate::anyhow_tauri::bail!("Showcase of the .bail!()")
/// }
/// ```
#[macro_export]
macro_rules! bail {
...
/// ```
/// #[tauri::command]
/// fn test_ensure() -> crate::anyhow_tauri::TAResult<String> {
///     crate::anyhow_tauri::ensure!(1 == 2); // this should throw
